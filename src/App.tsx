...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, subDays, startOfMonth, startOfYear } from 'date-fns';
...
export default function App() {
  ...
  // --- [수정] 데이터 로직: 서버 대신 LocalStorage 사용 ---
  
  useEffect(() => {
    // 1. 서비스 워커 등록 (PWA 설치를 위해 필수)
  ...
  const loadData = () => {
    const savedRuns = localStorage.getItem('stridetrack_data');
    if (savedRuns) {
  ...
  // 서버에서 하던 통계 계산을 클라이언트에서 직접 수행 (차트 기능을 위해)
  const calculateStats = (allRuns: Run[]) => {
    const now = new Date();
    
    // 단순화된 통계 계산 로직 (원본 차트 호환용)
    ...
    setStats({
      daily,
      weekly: [{ week: 'This Week', distance: daily.reduce((sum, d) => sum + d.distance, 0), steps: daily.reduce((sum, d) => sum + d.steps, 0) }],
      monthly: [{ month: format(now, 'MMM'), distance: daily.reduce((sum, d) => sum + d.distance, 0), steps: daily.reduce((sum, d) => sum + d.steps, 0) }],
      yearly: [{ year: format(now, 'yyyy'), distance: daily.reduce((sum, d) => sum + d.distance, 0), steps: daily.reduce((sum, d) => sum + d.steps, 0) }]
    });
  };

  const saveRunLocally = () => {
    const newRun: Run = {
      id: Date.now(),
      distance,
      duration,
    ...
    setRuns(updatedRuns);
    localStorage.setItem('stridetrack_data', JSON.stringify(updatedRuns));
      ...
      watchId.current = navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (lastPosition.current) {
            const d = calculateDistance(lastPosition.current.latitude, lastPosition.current.longitude, position.coords.latitude, position.coords.longitude);
        ...
        (err) => { setGpsStatus('error'); setErrorMsg(`GPS Error: ${err.message}`); },
        ...
        <div className="space-y-3">
          {runs.slice(0, 3).map(run => (
            <div key={run.id} className="bg-white p-4 rounded-xl border border-slate-100 flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400"><MapPin size={20} /></div>
  ...
  const renderActiveRun = () => (
    <div className="h-full flex flex-col bg-slate-900 text-white p-8">
      ...
      <div className="flex gap-4 pb-8">
        <button onClick={() => setAudioEnabled(!audioEnabled)} className={cn("p-4 rounded-2xl", audioEnabled ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-800 text-slate-500")}>
          {audioEnabled ? <Volume2 size={24} /> : <VolumeX size={24} />}
        </button>
        <button onClick={handleFinish} className="flex-1 bg-rose-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2"><Square size={20} fill="currentColor" />FINISH RUN</button>
      </div>
    </div>
  );

  // Statistics, History, Settings 렌더링 함수는 원본 코드를 그대로 사용하시되, 
  // 위에서 수정된 stats 상태를 기반으로 차트가 그려지게 됩니다.
    ...
    <div className="max-w-md mx-auto h-screen bg-slate-50 overflow-y-auto font-sans relative">
      <AnimatePresence mode="wait">
        <motion.div key={view} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full">
          ...
          {/* history, stats, settings는 원본 코드의 render 부분을 그대로 넣으시면 됩니다. */}
      ...
      {view !== 'active-run' && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/80 backdrop-blur-md border-t border-slate-100 px-6 py-3 flex justify-between items-center">
          <button onClick={() => setView('dashboard')} className={cn("p-2 flex flex-col items-center gap-1", view === 'dashboard' ? "text-emerald-600" : "text-slate-400")}><Play size={20} /><span className="text-[10px] font-bold uppercase">Run</span></button>
          <button onClick={() => setView('stats')} className={cn("p-2 flex flex-col items-center gap-1", view === 'stats' ? "text-emerald-600" : "text-slate-400")}><TrendingUp size={20} /><span className="text-[10px] font-bold uppercase">Stats</span></button>
          <button onClick={() => setView('history')} className={cn("p-2 flex flex-col items-center gap-1", view === 'history' ? "text-emerald-600" : "text-slate-400")}><History size={20} /><span className="text-[10px] font-bold uppercase">History</span></button>
          <button onClick={() => setView('settings')} className={cn("p-2 flex flex-col items-center gap-1", view === 'settings' ? "text-emerald-600" : "text-slate-400")}><SettingsIcon size={20} /><span className="text-[10px] font-bold uppercase">Settings</span></button>
        </div>
      )}
    </div>
  );
